...
def cosine_loss(logits, labels, num_classes=2, class_weights=None):
    """
    PyTorch implementation of the cosine loss from cvjena/semantic-embeddings.
    It L2-normalizes logits and one-hot labels, then computes 1 - dot_product.
    """
    # 1. L2-normalize the model's logits
    normalized_logits = torch.nn.functional.normalize(logits, p=2, dim=-1)
    
    # 2. Convert integer labels to one-hot vectors and L2-normalize them
    # The one-hot vectors are already L2-normalized since they are unit vectors.
    y_true_one_hot = torch.nn.functional.one_hot(labels, num_classes=num_classes).float()
    
    # 3. Calculate the dot product (cosine similarity)
    cosine_similarity = torch.sum(y_true_one_hot * normalized_logits, dim=-1)
    
    # 4. The loss is 1 - cosine_similarity
    loss = 1.0 - cosine_similarity
    
    # 5. Apply class weights if provided
    if class_weights is not None:
        # Gather the weights for each sample in the batch
        weights = class_weights.gather(0, labels)
        loss = loss * weights
...
def main():
    ...
    with accelerator.main_process_first():
        if args.max_train_samples is not None:
            dataset["train"] = dataset["train"].shuffle(seed=args.seed).select(range(args.max_train_samples)) # type: ignore
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                labels = batch["labels"]
                loss = cosine_loss(logits, labels, num_classes=2, class_weights=class_weights)
